...
메인 수직 맵 데이터는 Tiled Map Editor에서 열 수 있는 다음 파일로 관리한다.

```text
public/assets/tilemaps/initial-vertical.tmj
```

현재는 도트 타일셋 반영 전 단계라 Tiled의 타일 레이어로 지형 덩어리를 관리하고,
오브젝트 레이어는 필드 메타 정보, 시작점, 도착점, 접면 제어선 같은 보조 데이터만
관리한다.

픽셀 단위, 타일 크기, 기본 오브젝트 크기, 달팽이 픽셀 기준은 `notes/pixel-map-guidelines.md`에서 관리한다.
...
- 세부 게임 수치는 확정 전까지 초기 튜닝값으로 표시한다.
